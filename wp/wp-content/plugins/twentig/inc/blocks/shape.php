<?php
/**
 * Shape customizations for the `core/image`, `core/cover`, `core/post-featured-image`, and `core/group` blocks.
 *
 * @package twentig
 */
 ...
 *
 * @return array Array of shapes with their SVG path data.
 */
...
function twentig_enqueue_shape_svg( $shape ) {
	global $twentig_shapes;

	if ( ! isset( $twentig_shapes ) ) {
		$twentig_shapes = array();
	}

	if ( ! in_array( $shape, $twentig_shapes, true ) ) {
		$twentig_shapes[] = $shape;
		echo twentig_render_shape_svg( $shape ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- SVG contains hardcoded paths from twentig_get_shapes() with escaped attributes.
	}
}
 ...
 *
 * @param string $shape The selected shape.
 * @param string $env   Environment context. Accepts 'front' or 'editor'. Default 'front'.
 * @return string|null SVG markup or null if shape not found.
 */
...
function twentig_enqueue_group_shape_css( $shape ) {
	global $twentig_group_shapes;

	$css = '';

	if ( ! isset( $twentig_group_shapes ) ) {
		$twentig_group_shapes = array();
		$css            .= '
	...
	if ( ! in_array( $shape, $twentig_group_shapes, true ) ) {
		$twentig_group_shapes[] = $shape;
